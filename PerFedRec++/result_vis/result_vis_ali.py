...
filepath = './gowalla/train_PerFedRec_item.pt'
post_fix = ''
...
keep_cols = np.random.choice(item_emb.shape[0], size=1000, replace=False)
feature_set_1 = item_emb[ keep_cols, :]

keep_cols = np.random.choice(item_emb.shape[0], size=1000, replace=False)
feature_set_2 = item_emb[ keep_cols, :]
positive_pair_distances = np.linalg.norm(feature_set_1 - feature_set_2, axis=1)
plt.figure(figsize=(9, 7))
plt.hist(positive_pair_distances, bins=30, alpha=0.5, color='blue')
mean_positive_distance = np.mean(positive_pair_distances)
plt.axvline(mean_positive_distance, color='k', linestyle='dashed', linewidth=1, label=f'Mean: {mean_positive_distance:.2f}')
plt.title('Alignment')
plt.xlabel('\u2113\u2082 Distances')
plt.ylabel('Counts')
plt.legend()

plt.savefig(''+f'gowalla_item_ali{post_fix}'+'.png')
...
angles = np.arctan2(item_emb_tsne[:, 1], item_emb_tsne[:, 0])  # Angles in radians
angles = np.mod(angles, 2 * np.pi)  # Convert angles to range 0 to 2*pi
